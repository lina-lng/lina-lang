...
let print_ffi_spec spec =
  ...
  in
  Printf.printf "kind=%s lua_name=%s variadic=%b nullable=%b pcall=%b arity=%d\n"
    kind_str spec.ffi_lua_name spec.ffi_is_variadic spec.ffi_return_nullable spec.ffi_return_pcall spec.ffi_arity
...
let%expect_test "parse @module attribute" =
  ...
  [%expect {| kind=Module(socket) lua_name=test variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "parse @val attribute" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Global lua_name=test variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "parse @scope single string" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Global(math) lua_name=test variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "parse @scope string list" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Global(table.insert) lua_name=test variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "parse @send attribute" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Method lua_name=test variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "parse @get attribute" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Getter lua_name=test variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "parse @set attribute with arity 2" =
  ...
  (match parse_attrs_with_arity 2 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Setter lua_name=test variadic=false nullable=false pcall=false arity=2 |}]
...
let%expect_test "parse @get_index attribute with arity 2" =
  ...
  (match parse_attrs_with_arity 2 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=IndexGetter lua_name=test variadic=false nullable=false pcall=false arity=2 |}]
...
let%expect_test "parse @set_index attribute with arity 3" =
  ...
  (match parse_attrs_with_arity 3 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=IndexSetter lua_name=test variadic=false nullable=false pcall=false arity=3 |}]
...
let%expect_test "parse @new attribute" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Constructor lua_name=test variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "parse @variadic attribute" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Global lua_name=test variadic=true nullable=false pcall=false arity=1 |}]
...
let%expect_test "parse @return(nullable) attribute" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Global lua_name=test variadic=false nullable=true pcall=false arity=1 |}]
...
let%expect_test "parse @as attribute" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Global lua_name=lua_print variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "error: @return with invalid payload" =
  ...
  (match parse_attrs attrs with
   | Ok _ -> print_endline "ERROR: should have failed"
   | Error e -> print_error e);
  [%expect {| Invalid payload for @return: expected "nullable" or "pcall", got "invalid" |}]
...
let%expect_test "error: @return without payload" =
  ...
  (match parse_attrs attrs with
   | Ok _ -> print_endline "ERROR: should have failed"
   | Error e -> print_error e);
  [%expect {| Invalid payload for @return: expected argument: @return(nullable) or @return(pcall) |}]
...
let%expect_test "valid: @module with @as" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Module(socket) lua_name=connect_tcp variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "valid: @send with @variadic" =
  ...
  (match parse_attrs_with_arity 2 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Method lua_name=test variadic=true nullable=false pcall=false arity=2 |}]
...
let%expect_test "valid: @send with @return(nullable)" =
  ...
  (match parse_attrs_with_arity 2 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Method lua_name=test variadic=false nullable=true pcall=false arity=2 |}]
...
let%expect_test "valid: @module with @return(nullable)" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Module(db) lua_name=test variadic=false nullable=true pcall=false arity=1 |}]

let%expect_test "valid: @val with @return(pcall)" =
  let attrs = [
    make_attr "val" None;
    make_attr "return" (Some (Parsing_ffi.Attributes.PayloadIdent "pcall"));
  ] in
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Global lua_name=test variadic=false nullable=false pcall=true arity=1 |}]

let%expect_test "valid: @val with @return(pcall) string payload" =
  let attrs = [
    make_attr "val" None;
    make_attr "return" (Some (Parsing_ffi.Attributes.PayloadString "pcall"));
  ] in
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
...
let%expect_test "valid: @val with @scope and @as" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Global(os) lua_name=get_time variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "valid: multiple modifiers" =
  ...
  (match parse_attrs_with_arity 2 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Method lua_name=method_name variadic=true nullable=true pcall=false arity=2 |}]
...
(** Compile source and return Lua code (without stdlib prelude) or error.
    Strips the stdlib prelude to keep expected outputs small and focused on
    the actual FFI-generated code being tested. *)
let compile source =
...
let%expect_test "valid: @send with arity 3" =
  ...
  (match parse_attrs_with_arity 3 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Method lua_name=test variadic=false nullable=false pcall=false arity=3 |}]
...
let%expect_test "valid: @new with arity 0" =
  ...
  (match parse_attrs_with_arity 0 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Constructor lua_name=test variadic=false nullable=false pcall=false arity=0 |}]
...
let%expect_test "valid: @module with arity 0" =
  ...
  (match parse_attrs_with_arity 0 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Module(socket) lua_name=test variadic=false nullable=false pcall=false arity=0 |}]
...
let%expect_test "valid: @val with arity 0" =
  ...
  (match parse_attrs_with_arity 0 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Global lua_name=test variadic=false nullable=false pcall=false arity=0 |}]
...
let%expect_test "valid: @module with @variadic" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Module(utils) lua_name=test variadic=true nullable=false pcall=false arity=1 |}]
...
let%expect_test "valid: @new with @as" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Constructor lua_name=create variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "valid: @new with @variadic" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Constructor lua_name=test variadic=true nullable=false pcall=false arity=1 |}]
...
let%expect_test "valid: @get with @as" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Getter lua_name=length variadic=false nullable=false pcall=false arity=1 |}]
...
let%expect_test "valid: @set with @as" =
  ...
  (match parse_attrs_with_arity 2 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Setter lua_name=value variadic=false nullable=false pcall=false arity=2 |}]
...
let%expect_test "valid: @get_index with @as" =
  ...
  (match parse_attrs_with_arity 2 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=IndexGetter lua_name=at variadic=false nullable=false pcall=false arity=2 |}]
...
let%expect_test "valid: @set_index with @as" =
  ...
  (match parse_attrs_with_arity 3 attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=IndexSetter lua_name=put variadic=false nullable=false pcall=false arity=3 |}]
...
let%expect_test "valid: @get with @return(nullable)" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Getter lua_name=test variadic=false nullable=true pcall=false arity=1 |}]
...
let%expect_test "valid: @new with @return(nullable)" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Constructor lua_name=test variadic=false nullable=true pcall=false arity=1 |}]
...
let%expect_test "valid: @val with @scope and @variadic" =
  ...
  (match parse_attrs attrs with
   | Ok spec -> print_ffi_spec spec
   | Error e -> print_error e);
  [%expect {| kind=Global(string) lua_name=test variadic=true nullable=false pcall=false arity=1 |}]
