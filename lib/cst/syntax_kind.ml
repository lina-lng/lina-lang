...
let token_category = function
  (* Keywords *)
  | TK_LET | TK_REC | TK_IN | TK_FUN | TK_FUNCTION
  | TK_IF | TK_THEN | TK_ELSE | TK_TYPE | TK_OF | TK_AND
  | TK_AS | TK_MATCH | TK_WITH | TK_WHEN
  | TK_MODULE | TK_STRUCT | TK_END | TK_SIG | TK_FUNCTOR
  | TK_OPEN | TK_INCLUDE | TK_VAL | TK_PRIVATE | TK_CONSTRAINT
  | TK_EXTERNAL | TK_TRUE | TK_FALSE | TK_REF | TK_ASSERT
  | TK_WHILE | TK_DO | TK_DONE | TK_FOR | TK_TO | TK_DOWNTO -> Keyword
  ...
  (* Operators *)
  | TK_STAR | TK_PLUS | TK_MINUS | TK_MINUSDOT | TK_SLASH | TK_MOD | TK_CARET
  ...
  (* Identifiers *)
  | TK_LOWERCASE_IDENT | TK_UPPERCASE_IDENT
  | TK_TYPE_VARIABLE | TK_BACKTICK_TAG -> Identifier

  (* Delimiters *)
  | TK_LPAREN | TK_RPAREN | TK_LBRACKET
  | TK_RBRACKET | TK_LBRACE | TK_RBRACE -> Delimiter

  (* Trivia *)
  | TK_WHITESPACE | TK_NEWLINE
  | TK_LINE_COMMENT | TK_BLOCK_COMMENT -> Trivia
...
let is_token = function
  ...
  | NK_PAREN_EXPR | NK_RECORD_FIELD | NK_MATCH_ARM
  ...
  | NK_TYPE_ARROW | NK_TYPE_RECORD | NK_PAREN_TYPE
  | NK_TYPE_RECORD_FIELD
  | NK_BINDING | NK_TYPE_DECLARATION | NK_CONSTRUCTOR_DECLARATION
  | NK_EXTERNAL_DECLARATION
  ...
  | NK_STRUCTURE | NK_MODULE_PATH | NK_FUNCTOR_EXPR
  | NK_MODULE_APPLY | NK_MODULE_CONSTRAINT
  | NK_SIGNATURE | NK_MODULE_TYPE_PATH | NK_FUNCTOR_TYPE | NK_MODULE_TYPE_WITH
  ...
  | NK_FUNCTOR_PARAMETER | NK_WITH_TYPE_CONSTRAINT | NK_WITH_MODULE_CONSTRAINT
  | NK_MODULE_BINDING
  | NK_LONGIDENT | NK_ATTRIBUTE | NK_ATTRIBUTE_PAYLOAD
  | NK_TYPE_PARAMETERS | NK_ARGUMENT_LIST -> false
  | _ -> true

let is_node kind = not (is_token kind)

let is_trivia kind = token_category kind = Trivia

let is_comment = function
  | TK_LINE_COMMENT | TK_BLOCK_COMMENT -> true
  | _ -> false

let is_keyword kind = token_category kind = Keyword
let is_punctuation kind = token_category kind = Punctuation
let is_operator kind = token_category kind = Operator
